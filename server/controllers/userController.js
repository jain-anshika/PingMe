...
export const login = async ()=>{
    try {
            ...
            res.json({sucess:true, userData, token, message:"Login successful"})
    } catch (error) {
        console.log(error.message);
        res.json({sucess:false, message:error.message})
    }
...
export const updateProfile = async(req,res)=>{
    try {
        ...
        if(!profilePic){
            ...
            updatedUser = await User.findByIdAndUpdate(userId, {prolfilePic: upload.secure_url, bio, fullName}, {new:true});
        }
